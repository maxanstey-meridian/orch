...
import { runInit, profileToMarkdown, type InitProfile, type AskFn, type AskHandle } from "../src/init.js";
...
describe("init profile + fingerprint integration", () => {
  ...
  it("prepends init profile to generated brief", async () => {
    ...
    // Init profile appears at the start of the brief
    expect(result.brief.startsWith("## Project Profile (from init)")).toBe(true);
    expect(result.brief).toContain("- **Language:** TypeScript");
    expect(result.brief).toContain("- **Framework:** Express");
    // Generated brief follows
    expect(result.brief).toContain("# Codebase Brief");
  });

  it("generates brief without init prefix when no init-profile.md exists", async () => {
    const orchDir = join(tmpDir, ".orch");
    ...
    expect(result.brief).not.toContain("Project Profile (from init)");
    expect(result.brief).toContain("# Codebase Brief");
  });

  it("forceRefresh bypasses cached brief when init profile changes", async () => {
    const orchDir = join(tmpDir, ".orch");
    mkdirSync(orchDir, { recursive: true });
    ...
    expect(first.brief).toContain("- **Framework:** Express");
    ...
    expect(cached.brief).toContain("- **Framework:** Express"); // stale cache
    ...
    expect(refreshed.brief).not.toContain("- **Framework:** Express");
  });
});
