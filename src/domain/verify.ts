...
export const parseVerifyResult = (text: string): VerifyResult => {
  ...
  if (!verifyJsonText) {
    return INVALID_VERIFY_RESULT(text, "Verifier output missing required VERIFY_JSON block");
  }
  ...
  if (!parsed.success) {
    ...
    return INVALID_VERIFY_RESULT(text, `Verifier output failed VERIFY_JSON validation: ${issues}`);
  }
