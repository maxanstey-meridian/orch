...
const joinFailureParts = (parts: readonly string[]): string =>
  parts
    .map((part) => part.trim())
...
export const collectAgentFailureText = (result: AgentResult, stderr: string): string =>
  joinFailureParts([result.assistantText, result.resultText, stderr]);
