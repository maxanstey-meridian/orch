...
export const buildTriagePrompt = (input: ExecutionUnitTriageInput): string => `You are the runtime policy classifier for a TDD orchestration pipeline.
...
export const parseTriageResult = (text: string): BoundaryTriageResult => {
  try {
    ...
    const triage = triageSchema.safeParse(parsed);
    if (triage.success) {
      return triage.data;
    }
