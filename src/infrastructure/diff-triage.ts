...
export const buildTriagePrompt = (diff: string): string => `You are a change classifier for a TDD orchestration pipeline.
...
export const parseTriageResult = (text: string): TriageResult => {
  try {
    ...
    if (json === null) return FULL_TRIAGE;

    const parsed = JSON.parse(json) as unknown;
    if (!isParsedTriageResult(parsed)) return FULL_TRIAGE;
