...
export const ts = (): string => {
  const date = new Date();
  return `${a.dim}${date.toLocaleTimeString("en-GB", { hour12: false })}${a.reset}`;
};
...
export const logSection = (log: LogFn, title: string): void => {
...
export const printSliceIntro = (log: LogFn, slice: Slice): void => {
...
export const printSliceContent = (log: LogFn, slice: Slice): void => {
  log(`\n${a.bold}${a.white}┌─ Slice ${slice.number}: ${slice.title}${a.reset}`);
  if (slice.why) {
    log(`${a.dim}│  ${slice.why.trim()}${a.reset}`);
  }
  if (slice.files.length > 0) {
    ...
    for (const file of slice.files) {
      log(`${a.dim}│    ${a.reset}${file.path} (${file.action})`);
    }
  }
  if (slice.details) {
    log(`${a.dim}│${a.reset}`);
...
export const printSliceSummary = (
  log: LogFn,
  sliceNumber: number,
  summary: string,
): void => {
  ...
  for (const line of summary.trim().split("\n")) {
    const formatted = line
      .replace(/^## (.+)/u, `${a.bold}${a.white}│ $1${a.reset}`)
      .replace(/^- (.+)/u, `${a.dim}│${a.reset}  - $1`)
      .replace(/^(?!│)(.+)/u, `${a.dim}│${a.reset}  $1`);
    log(formatted);
  }
...
export const formatPlanSummary = (log: LogFn, groups: readonly Group[]): void => {
  for (const group of groups) {
    log(`\n${a.bold}${a.cyan}┌─ ${group.name}${a.reset}`);
    for (const slice of group.slices) {
      log(`${a.dim}│${a.reset}  ${a.bold}Slice ${slice.number}: ${slice.title}${a.reset}`);
      if (slice.why) {
        log(`${a.dim}│     ${slice.why}${a.reset}`);
      }
      const fileList = slice.files.map((file) => `${file.path} (${file.action})`).join(", ");
      log(`${a.dim}│     Files: ${fileList}${a.reset}`);
      if (slice.tests) {
        log(`${a.dim}│     Tests: ${slice.tests}${a.reset}`);
      }
    }
    log(`${a.dim}└──${a.reset}`);
  }
...
export const printStartupBanner = (log: LogFn, opts: BannerOpts): void => {
  ...
  log("");
  for (let index = 0; index < opts.groups.length; index += 1) {
    const group = opts.groups[index];
    const slices = group.slices.map((slice) => `${slice.number}`).join(", ");
    const marker = index === 0 ? `${a.bold}▸${a.reset}` : " ";
    log(
      `   ${marker} ${a.dim}${String(index + 1).padStart(2)}.${a.reset} ${index === 0 ? a.bold : a.dim}${group.name}${a.reset} ${a.dim}(${slices})${a.reset}`,
    );
  }
  log("");
};
