...
export type HudMode = "status" | "guide" | "interrupt" | "ask";

export type HudViewProps = {
...
export const buildStatusLine = (state: HudState, columns: number): string => {
  ...
  if (state.executionMode) {
    parts.push(`Mode: ${state.executionMode}`);
  }
...
export const HudView = ({
  ...
  const status = buildStatusLine(state, columns - 2);
...
const App = () => {
  ...
  const [mode, setMode] = useState<HudMode>("status");
