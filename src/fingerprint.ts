...
const dirTree = (root: string, dir: string, depth = 1): string[] => {
  ...
  const dirs = entries.filter((e) => e.isDirectory() && !e.name.startsWith('.') && e.name !== 'node_modules' && e.name !== 'bin' && e.name !== 'obj');
  const result: string[] = [];
  for (const d of dirs) {
    result.push(`${dir}/${d.name}/`);
    if (depth > 1) result.push(...dirTree(root, `${dir}/${d.name}`, depth - 1));
  }
  return result;
};
...
export const detectStack = (root: string): StackInfo => {
  ...
  if (csprojs.length > 0) {
    ...
    const hasAspNet = uniqueDeps.some((d) => d.includes('AspNetCore')) || allContent.includes('Microsoft.NET.Sdk.Web');
    const hasRoslyn = uniqueDeps.some((d) => d.includes('CodeAnalysis'));
    const framework = hasAspNet ? 'ASP.NET Core' : hasRoslyn ? 'Roslyn analysis tool' : 'console/library';
...
export const detectArchPattern = (folders: string[]): string => {
  ...
  const hasApplication = flat.some((f) => f.includes('/application') || f.includes('/import') || f.includes('/mapping'));
  const hasPipeline = flat.some((f) => f.includes('/pipeline'));
  const hasPorts = flat.some((f) => f.includes('/ports/'));

  if (hasModules && hasPorts) return 'VSA + Clean Architecture modular monolith';
  if (hasDomain && hasApplication && hasPorts) return 'Clean Architecture (ports & adapters)';
  if (hasPipeline) return 'Pipeline architecture (stateless stages + mutable accumulator)';
  if (hasDomain && hasApplication) return 'Layered architecture';
...
export const detectTestStyle = (root: string): string => {
  ...
  if (testFiles.length === 0) {
    ...
    if (tsTests.length === 0) return 'No tests found';
    const sample = tryRead(join(root, tsTests[0]));
    if (sample.includes('vitest')) return 'Vitest';
    if (sample.includes('jest')) return 'Jest';
    return 'Unknown TS test framework';
  }

  const sample = tryRead(join(root, testFiles[0]));
  if (sample.includes('xUnit') || sample.includes('[Fact]') || sample.includes('[Theory]')) return 'xUnit';
  if (sample.includes('[Test]') || sample.includes('[TestFixture]')) return 'NUnit';
  if (sample.includes('[TestMethod]')) return 'MSTest';
...
export const detectCodePatterns = (root: string, files: string[]): string[] => {
  ...
  for (const f of sampleFiles) {
    ...
    if (!seen.has('no-di') && sampleFiles.indexOf(f) > 5 && !content.includes('IServiceCollection') && !content.includes('[Inject]')) {
      seen.add('no-di');
...
export const detectAntiPatterns = (root: string): string[] => {
  ...
  for (const f of csFiles.slice(0, 20)) {
    ...
    if (/\bdynamic\b/.test(content) || /\bobject\?\b/.test(content)) hasDynamic = true;
  }

  if (!hasInterface) anti.push('No service interfaces — stateless components are static classes called directly');
  if (!hasInheritance) anti.push('No class inheritance hierarchies — sealed records for polymorphism');
  if (!hasDynamic) anti.push('No dynamic/object? value carriers — everything strongly typed');
...
export const sampleFlow = (root: string): string => {
  ...
  for (const f of candidates) {
    ...
    const classStart = lines.findIndex((l) => /public static class|public sealed class|export (const|function|async)/.test(l));
    if (classStart >= 0) {
      return lines.slice(classStart, classStart + MAX_SAMPLE_LINES).join('\n').trim();
    }
  }
...
export const generateBrief = (root: string, precomputed?: { stack: StackInfo; testStyle: string }): string => {
  ...
  if (stack.deps.length > 0) {
    const keyDeps = stack.deps.filter((d) =>
      !d.includes('Test') && !d.includes('xunit') && !d.includes('coverlet'),
    ).slice(0, 6);
    if (keyDeps.length > 0) {
  ...
  if (folders.length > 0) {
    ...
    for (const f of folders) {
      ...
      if (parts.length > 2) grouped.get(key)!.push(parts.slice(2).join('/'));
    }
  ...
  if (antiPatterns.length > 0) {
    ...
    lines.push('');
    for (const a of antiPatterns) lines.push(`- ${a}`);
    lines.push('');
  }
  ...
  if (fileExists(join(root, 'CLAUDE.md'))) {
    lines.push('## See also');
    lines.push('');
    lines.push('CLAUDE.md has authoritative architecture decisions, change ripple map, and hard rules.');
    lines.push('This brief is a structural snapshot — CLAUDE.md is the source of truth for intent.');
    lines.push('');
  }
